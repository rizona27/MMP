...
struct AddHoldingView: View {
    ...
    private func saveHolding() {
        ...
        fundService.addLog("新增持仓: \(newHolding.fundCode) for \(newHolding.clientName)。")
        ...
        Task { @MainActor in
            if let index = dataManager.holdings.firstIndex(where: { $0.id == newHolding.id }) {
                ...
                fundService.addLog("基金 \(holdingToUpdate.fundCode) 信息已刷新。")
            } else {
                fundService.addLog("警告: 未找到刚刚添加的持仓以刷新信息。")
            }
        }
        dismiss()
    }
