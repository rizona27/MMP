...
class FundService: ObservableObject {
    ...
    @Published var logMessages: [String] = [] // 用于存储 API 请求日志
    ...
    func fetchFundInfo(code: String) async -> FundHolding {
        ...
        if let cachedData = getFromCache(code: code) {
            ...
            if isSameNavDay && isCacheFresh {
                addLog("基金代码 \(code): 从主缓存中获取数据，净值日期为今日且缓存未过期。")
                return cachedData.holding
            } else {
                addLog("基金代码 \(code): 主缓存数据净值日期非今日或缓存已过期，尝试更新。")
            }
        }
        
        // 2. 主缓存不存在或已过期，从 API 并发获取
        addLog("基金代码 \(code): 主缓存不可用，开始并发尝试从API获取。")
        ...
        await withTaskGroup(of: FundHolding?.self) { group in
            for (index, urlStringTemplate) in apiURLs.enumerated() {
                group.addTask {
                    ...
                    guard let url = URL(string: urlString) else {
                        self.addLog("基金代码 \(code): API \(index + 1) URL无效: \(urlString)")
                        return nil
                    }
                    
                    self.addLog("基金代码 \(code): 尝试从 API \(index + 1) 获取数据: \(urlString)")
                    
                    let randomDelay = Double.random(in: 0.5...1.0)
                    self.addLog("基金代码 \(code): API \(index + 1) 将延迟 \(String(format: "%.2f", randomDelay)) 秒。")
                    ...
                    do {
                        ...
                        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                            self.addLog("基金代码 \(code): API \(index + 1) 响应状态码非200: \((response as? HTTPURLResponse)?.statusCode ?? -1)")
                            return nil
                        }
                        
                        if let result = await self.parseFundData(data: data, apiIndex: index, code: code) {
                            self.addLog("基金代码 \(code): API \(index + 1) 解析成功。")
                            return result
                        } else {
                            self.addLog("基金代码 \(code): API \(index + 1) 数据解析失败。")
                            return nil
                        }
                    } catch {
                        self.addLog("基金代码 \(code): API \(index + 1) 请求失败: \(error.localizedDescription)")
                        return nil
                    }
                }
            }
            ...
            for await result in group {
                if let validHolding = result, validHolding.isValid {
        ...
        if let dataFromAPI = fetchedHolding, dataFromAPI.isValid {
            ...
            addLog("基金代码 \(code): 成功获取有效数据并更新主缓存。")
        } else {
            if let cachedData = getFromCache(code: code) {
                addLog("基金代码 \(code): 新数据获取失败，返回旧的主缓存数据。")
                finalHolding = cachedData.holding
                finalHolding.isValid = false
            } else {
                addLog("基金代码 \(code): 未能获取任何有效数据，也没有可用主缓存。")
                finalHolding.isValid = false
            }
        }
                   ...
                   let match = regex.firstMatch(in: string, options: [], range: NSRange(string.startIndex..., in: string)) {
                    ...
                    if let jsonData = jsonString.data(using: .utf8) {
                        do {
                            ...
                            addLog("基金代码 \(code): API 1 JSON 解析错误: \(error.localizedDescription)")
                        }
                    }
                }
            }
            ...
            if let string = String(data: data, encoding: .utf8) {
                ...
                if let syl6mRegex = try? NSRegularExpression(pattern: #"var syl_6y="([\d.-]+)";"#, options: []),
                   let syl6mMatch = syl6mRegex.firstMatch(in: string, options: [], range: NSRange(string.startIndex..., in: string)) {
                    ...
                    if let value = Double(valueString) {
                        holding.navReturn6m = value
                    }
                }
                
                // 确保使用 syl_1n 而不是 syl_1y
    ...
    private func saveToCache(holding: FundHolding) {
        cacheQueue.sync {
            ...
            addLog("基金代码 \(holding.fundCode): 数据已存入主缓存。")
        }
    }
    ...
    private func loadCacheFromUserDefaults() {
        if let savedData = UserDefaults.standard.data(forKey: userDefaultsKey) {
            do {
                ...
                addLog("加载主缓存失败: \(error.localizedDescription)")
            }
        } else {
            addLog("UserDefaults 中没有找到主缓存数据。")
        }
    }
    ...
    private func persistCacheToUserDefaults() {
        do {
            ...
            addLog("持久化主缓存失败: \(error.localizedDescription)")
        }
    }

    // 日志记录方法
    internal func addLog(_ message: String) {
        DispatchQueue.main.async {
            let timestamp = FundService.dateFormatterYYYY_MM_DD.string(from: Date())
